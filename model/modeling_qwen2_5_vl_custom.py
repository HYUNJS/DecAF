...
class Qwen2_5_VLFlashAttention2Custom(Qwen2_5_VLFlashAttention2):
...
class Qwen2_5_VLDecoderLayerCustom(Qwen2_5_VLDecoderLayer):
    def __init__(self, config: Qwen2_5_VLTextConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Qwen2_5_VLFlashAttention2Custom(config, layer_idx)


class Qwen2_5_VLTextModelCustom(Qwen2_5_VLTextModel):
    def __init__(self, config: Qwen2_5_VLTextConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [
                Qwen2_5_VLDecoderLayerCustom(config, layer_idx)
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if output_attentions and (layer_idx >= self.attn_layer_idx_start and layer_idx < self.attn_layer_idx_end):
                with torch.no_grad():
                    ...
                    ##### Start: head-wise weighted aggregation
                    ## get max weight per each query
...
class Qwen2_5_VLModelCustom(Qwen2_5_VLModel):
...
class Qwen2_5_VLForConditionalGenerationCustom(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2_5_VLModelCustom(config)
