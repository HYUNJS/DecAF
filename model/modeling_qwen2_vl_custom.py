...
class Qwen2VLFlashAttention2Custom(Qwen2VLFlashAttention2):
...
class Qwen2VLDecoderLayerCustom(Qwen2VLDecoderLayer):
    def __init__(self, config: Qwen2VLTextConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Qwen2VLFlashAttention2Custom(config, layer_idx)


class Qwen2VLTextModelCustom(Qwen2VLTextModel):
    def __init__(self, config: Qwen2VLTextConfig):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [Qwen2VLDecoderLayerCustom(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_init()
    ...
    def forward(
        ...
        ##### Start: Added by JS
        if hidden_states.shape[1] == 1: # for decoding phase
            ...
            if output_attentions and (layer_idx >= self.attn_layer_idx_start and layer_idx < self.attn_layer_idx_end):
                with torch.no_grad():
                    ...
                    if visual_token_mask is not None:
                        ...
                        # head_weights = curr_attn[:, :, visual_token_end_idx+1:, visual_token_start_idx:visual_token_end_idx+1].max(dim=-1).values # (B, H, nt)
                    else:
...
class Qwen2VLModelCustom(Qwen2VLModel):
    def __init__(self, config):
        super().__init__(config)
        self.language_model = Qwen2VLTextModelCustom._from_config(config.text_config)
...
class Qwen2VLForConditionalGenerationCustom(Qwen2VLForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2VLModelCustom(config)
    ...
    def forward(
        ...
        ##### Start: Add by JS - consistency with Qwen2_5_VL
        if not return_dict:
