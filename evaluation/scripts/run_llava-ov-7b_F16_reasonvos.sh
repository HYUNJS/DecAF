##### Set Config
## Model
MODEL_NAME_LLAVAOV_7B="llava-ov-7b-rvos"
VERSION_LLAVAOV_7B="ckpts/llava-onevision-qwen2-7b-ov-hf"

# Select model and version
MODEL_NAME=${MODEL_NAME_LLAVAOV_7B}
VERSION=${VERSION_LLAVAOV_7B}
...
DATA_NAME=${DATA_NAME_REASONVOS}
DATA_CFG=${DATA_CFG_REASONVOS}
EVAL_DATA_CFG=${EVAL_DATA_CFG_REASONVOS}

## Attention Score
ATTN_NUM_FRAME=16
attn_layer_idx_start=14
attn_layer_idx_end=-1 # -1 means the last layer
ATTN_MS_CFG="--attn_max_batch 4"
...
for ((i=0; i<$SUBSET_NUM; i++)); do
    CUDA_VISIBLE_DEVICES=${GPU_LIST[i]} PYTHONPATH=$(pwd) python evaluation/inference_attn_map_llavaov.py \
