...
ATTN_MS_CFG="--attn_max_token 400 --attn_resize_scale 2 --attn_max_batch 8"
...
for ((i=0; i<$SUBSET_NUM; i++)); do
    CUDA_VISIBLE_DEVICES=${GPU_LIST[i]} PYTHONPATH=$(pwd) python evaluation/inference_attn_map_qwenvl.py \
...
GPU_LIST=(7 7)
SUBSET_NUM=${#GPU_LIST[@]}
SCORE_THR_LIST=(0.70 0.80 0.90)
POINT_THR_LIST=(0.70 0.80 0.90)
MASK_CFG="--subset_num=${SUBSET_NUM} --num_input_frames=${MASK_NUM_FRAME} --attn_save_path=${ATTN_FOLDER} --use_saved_attn --predict_sam_mask"
for POINT_THR in "${POINT_THR_LIST[@]}"; do
    for SCORE_THR in "${SCORE_THR_LIST[@]}"; do
        # Inference
        SAM_MASK_CFG="${MASK_CFG} --predict_sam_mask --final_score_threshold=${SCORE_THR} --point_threshold=${POINT_THR}"
        SAM_MASK_FOLDER="./outputs/${MODEL_NAME}/${ATTN_NAME}/${INF_NAME}-sam-mask_trk-thr-${SCORE_THR}-point-thr-${POINT_THR}/${DATA_NAME}"
